...
covidData = pd.read_csv("E:/MPS Data Science/Summer 2022/Data Challenge/Datasets/July 16th, "
                       "22/owid_covid-19-data_latest_covid-19-world-cases-deaths-testing.csv")
covidData = covidData[
...
countries = np.ndarray.tolist(pd.unique(covidData['location']))
for x in countries:
...
def build_graph(country, datePicker, predict):
    ...
    #                     mode='lines',
    #                  name='Actual Cases'))
    fig.update_layout(yaxis={'title': 'Forecasted Cases'})
    ...
    fig1.add_trace(go.Scatter(x=forecastedCovid['date'], y=forecastedCovid['Forecasted_Deaths'],
                              mode='lines',
                              name='Forecasted Deaths', line=go.scatter.Line(color="red")))
    # fig1.add_trace(go.Scatter(x=forecastedCovid['date'], y=forecastedCovid['new_deaths'],
    #              mode='lines',
    #             name='Actual Deaths'))
    fig1.update_layout(yaxis={'title': 'Forecasted Deaths'})
